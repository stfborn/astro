...
def calculate_positions(et):
    ...
    longitude = np.radians(8.27)
    elevation = 100.0

    # --------------------------------------------------------
    # Erde: WGS84-Ellipsoid
    # ----------------------------------------------------------

    re = 6378.137       # Äquatorradius [km]
    rp = 6356.7523142   # Polradius [km]

    flattening = (re - rp) / re

    # --------------------------------------------------------
    # Position des Beobachters im erdfesten System
    # --------------------------------------------------------

    observer_earth_fixed = spice.georec(
        longitude,
        latitude,
        elevation,
        re,
    ...
    # --------------------------------------------------------
    # Transformation Erde-fest -> J2000
    # --------------------------------------------------------

    earth_fixed_to_j2000 = spice.pxform(
        "ITRF93",
        "J2000",
        et
    )

    observer_earth_j2000 = (
        earth_fixed_to_j2000 @ observer_earth_fixed
    )

    # --------------------------------------------------------
    # Position des Erdmittelpunkts relativ zum SSB
    # --------------------------------------------------------
    ...
    for body_name, body in SOLSYS_DICT.items():
        ...
        # ----------------------------------------------------
        # Position des Körpers relativ zu Mainz
        # ----------------------------------------------------

        observer_to_body = (
            target_ssb - observer_ssb
        )

        distance_mainz = np.linalg.norm(
            observer_to_body
        )

        x, y, z = observer_to_body
...
title = plt.title(
    f"Sonnensystem – Beobachter Mainz – "
...
for body_name in SOLSYS_DICT:
    ...
    info_lines.append(
        ...
        f"Mainz {data['distance']/1e6:7.2f} Mio km   "
